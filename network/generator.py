...
class Generation_G(nn.Module):
    def __init__(self, n=16, w_noise =0.2, kernelsize=3, imdim=3, imsize=[192, 320]):
        ...
        # self.randconv = RandConv(3, n)
        self.adain2_fore = AdaIN2d_Noise(zdim, n)
        self.img = GET_IMAGE_Tanh1x1(n)

    def forward(self, x):
        # x = self.randconv(x)
        z1 = torch.randn(len(x), self.zdim).cuda()
        x = self.adain2_fore(x, z1, self.w)
        x = self.img(x)
        return x

class Generation_Phi(nn.Module):
