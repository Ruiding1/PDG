...
class ResNet(nn.Module):
    def __init__(self, block, layers,classes=7):
        ...
        self.p_logvar = nn.Sequential(nn.Linear(512 * block.expansion, 512),
                                      nn.ReLU())
        self.p_mu = nn.Sequential(nn.Linear(512, 512),
                                  nn.LeakyReLU())
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
...
def resnet18(pretrained=True, **kwargs):
    ...
    if pretrained:
        ...
        # model = torch.utils.model_zoo.load_url(ResNet18_Weights.IMAGENET1K_V1)
    return model
